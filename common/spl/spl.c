...
__weak void board_boot_order(u32 *spl_boot_list)
{
	...
	debug("jonn:2 bootlist[0]=%d",spl_boot_list[0]);
}
...
static struct spl_image_loader *spl_ll_find_loader(uint boot_device)
{
	...
	for (entry = drv; entry != drv + n_ents; entry++) {
		...
		debug("jonn: boot_device=%d entry->boot_device=%d \r\n",boot_device,entry->boot_device);
			     ...
			     u32 spl_boot_list[], int count)
{
	...
	for (i = 0; i < count && spl_boot_list[i] != BOOT_DEVICE_NONE; i++) {
		...
		if (loader)
			printf("jonn: Trying to boot from %s\n", loader->name);
		else
...
void board_init_r(gd_t *dummy1, ulong dummy2)
{
	...
	debug("jonn: spl_image.os=0x%x spl_image.entry_point=0x%x \r\n",spl_image.os,spl_image.entry_point);
	...
	debug("john: CONFIG_BOOTSTAGE_STASH_ADD=0x%x size=0x%x \r\n",CONFIG_BOOTSTAGE_STASH_ADD,CONFIG_BOOTSTAGE_STASH_SIZE);
